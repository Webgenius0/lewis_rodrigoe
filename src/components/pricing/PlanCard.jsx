...
const PlanCard = ({ title, price, features, buttonText, id }) => {
  ...
  const handleClick = () => {
    if (authData) {
      mutate(
        { ...authData, package_id: id },
        {
          onError: () => {
            navigate('/sign-up', { state: { authData } });
          },
        }
      );
    } else {
      navigate('/sign-up', { state: { package_id: id } });
    }
  };
    ...
    <div data-aos="zoom-in">
      ...
      >
        ...
        <div className="relative z-10  group-hover:bg-transparent transition-colors duration-300 p-0">
          ...
          <Button
            className={cn(
              'w-full mt-4 text-[#0C1523] text-right font-[Manrope] text-[14px] md:text-[15px] lg:text-[16px] not-italic font-medium leading-[24px] rounded-[12px] border-[1.5px] border-[solid] border-[#CBD2D9] px-3 md:px-4 lg:px-[24px] py-[18px]',
